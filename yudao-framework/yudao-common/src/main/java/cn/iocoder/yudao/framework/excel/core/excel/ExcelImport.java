...
import cn.iocoder.yudao.framework.common.util.lang.ObjectUtils;
import cn.iocoder.yudao.framework.common.util.reflect.ReflectUtils;
import cn.iocoder.yudao.framework.excel.core.callback.MethodCallback;
...
public class ExcelImport implements Closeable {
	 ...
	 * 用于清理缓存
	 */
	private Set<Class<?>> fieldTypes = CollUtil.newHashSet();
...
//		if (ef != null && (ef.type()==0 || ef.type()==type)){
	...
	public Object getCellValue(Row row, int column) {
		...
		try {
			...
			if (val != null && StrUtil.equalsAny(String.valueOf(val), "-", "/")) {
				return "";
			}
		} catch (Exception e) {
			return val;
	...
	public <E> List<E> getDataList(Class<E> cls, String... groups) throws InstantiationException, IllegalAccessException {
		return getDataList(cls,  0,false, groups);
	}
	public <E> List<E> getDataList(Class<E> cls, int dataType, String... groups) throws InstantiationException, IllegalAccessException {
		return getDataList(cls, dataType,false, groups);
	}
	...
	public <E> List<E> getDataList(Class<E> cls, final boolean isThrowException, String... groups) throws InstantiationException, IllegalAccessException {
		return getDataList(cls, 0, isThrowException, groups);
	}
	public <E> List<E> getDataList(Class<E> cls, int dataType, final boolean isThrowException, String... groups) throws InstantiationException, IllegalAccessException {
		return getDataList(cls, dataType, new MethodCallback() {
			@Override
	...
	public <E> List<E> getDataList(Class<E> cls, MethodCallback exceptionCallback, String... groups) throws InstantiationException, IllegalAccessException {
		return getDataList(cls, 0, exceptionCallback, groups);
	}
	public <E> List<E> getDataList(Class<E> cls, int dataType, MethodCallback exceptionCallback, String... groups) throws InstantiationException, IllegalAccessException {
				...
				if (val != null) {
					...
					if (StrUtil.isNotBlank(ef.dictType())) {
						try {
							...
							val = dictUtils.getMethod("parseDictDataValue", String.class, String.class,
